...
const ProfilePage = async () => {
  ...
  const bounties = await db.bountyTable.findMany({ where: { username: username } });
    ...
    <div>
      ...
      {totalBountyOfUser._sum.amount && totalBountyOfUser._sum.amount > 0 && (
        <div className="m-5">
          ...
          <form action={async () => 'tron/sendTrx'} className="flex flex-col gap-y-2 w-[400px]">
            <label htmlFor="text">Tron wallet address:</label>
            <input
              id="address"
              name="address"
              className="border-2 h-[50px]"
              placeholder="Your Tron wallet address"
            />
